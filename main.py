...
def show_day(message):
    ...
    if verify_user(message.from_user.id, cursor):
        ...
        if date < day_of_week():
            bot.send_message(message.chat.id, structure_output(query_day(date, (get_week()+1)%2, cursor)), 'MarkdownV2', disable_web_page_preview=True, reply_markup= markup)
        else:
            bot.send_message(message.chat.id, structure_output(query_day(date, get_week(), cursor)), 'MarkdownV2', disable_web_page_preview=True, reply_markup= markup)
    else:
        bot.send_message(message.chat.id, "У Вас немає доступу для користування ботом. Зв'яжіться з адміном якщо це помилка.", reply_markup= markup)
